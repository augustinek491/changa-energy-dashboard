...
export async function POST() {
  ...
  const [{ data: stations }, { data: liveRows }, { data: alarmRows }, { data: recipients }] =
    await Promise.all([
      ...
      db.from('station_live').select('station_id, pv_power_kw, health_state, status, today_kwh, month_kwh, total_kwh'),
      db.from('alarms')
        .select('alarm_name, severity, raised_at, stations!inner(name)')
        .is('resolved_at', null)
        .order('raised_at', { ascending: false })
        .limit(20),
      db.from('report_recipients').select('email, label').eq('active', true).order('created_at'),
    ]);

  if (!recipients?.length) {
    return NextResponse.json({ error: 'No active recipients configured' }, { status: 400 });
  }
  ...
  const reportStations: ReportStation[] = (stations ?? []).map(s => {
    ...
    return {
      ...
      location: s.location,
      status: st,
      pv_power_kw: l?.pv_power_kw ?? null,
  ...
  const html = await render(
    createElement(DailyReport, { date, generatedAt, summary, stations: reportStations, alarms: reportAlarms }),
  );

  const resend = new Resend(process.env.RESEND_API_KEY);
  const from = process.env.RESEND_FROM ?? 'Changa Energy <onboarding@resend.dev>';
  const toList = recipients.map(r => (r.label ? `${r.label} <${r.email}>` : r.email));

  const { data: sent, error: sendError } = await resend.emails.send({
    from,
    to: toList,
    subject: `Changa Energy — Solar Report · ${date}`,
    html,
  });
  ...
  return NextResponse.json({ sent: toList.length, recipients: toList, id: sent?.id });
}
